// Fixed Native Facebook Metrics Script
console.log('🔄 Pulling Native Facebook Metrics (Fixed)...\n');

async function pullNativeFacebookMetrics() {
  try {
    console.log('📱 Step 1: Getting Facebook connections...');
    
    // Use correct API endpoint
    const connectionsResponse = await fetch('/api/facebook?action=connections&workspaceId=c9a454c5-a5f3-42dd-9fbd-cedd4c1c49a9');
    ...
    for (const connection of connectionsData.connections) {
      ...
      console.log(`🔑 Token present: ${connection.token ? 'Yes' : 'No'}`);
      ...
      try {
        ...
        console.log('🔗 URL:', postsUrl.substring(0, 100) + '...');
        ...
        if (data.data && data.data.length > 0) {
          ...
          // Also update the FacebookEngagement component data
          window.facebookNativeMetrics = nativeMetrics;
          
          // Trigger refresh if FacebookEngagement component is listening
          if (window.updateFacebookEngagement) {
            window.updateFacebookEngagement(nativeMetrics);
          }
